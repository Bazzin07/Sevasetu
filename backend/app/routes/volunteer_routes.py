...
async def register_volunteer(data: VolunteerRegister, db: AsyncSession = Depends(get_db)):
    ...
    # Generate skill embedding for pgvector cosine matching
    skills_list = data.skills or []
    embed_text = " ".join(skills_list)
    if data.experience_text:
        embed_text = f"{embed_text} {data.experience_text}"
    vol_embedding = embedding_service.encode(embed_text) if embed_text.strip() else None

    volunteer = Volunteer(
        name=data.name, phone=data.phone, firebase_uid=data.firebase_uid,
        skills=skills_list, languages=data.languages or [],
